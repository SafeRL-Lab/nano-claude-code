...
- May 5, 2026: **Telegram bridge file round-trip + clickable permission prompts (#84) — `bridges/telegram.py` previously only had `_tg_send` (text via `sendMessage`), so when the model claimed it had "sent a file" it was just text and the `[approve][reject]` text in permission prompts only *looked* like buttons. Added `_tg_send_document` (multipart/form-data upload, 49 MB cap with explicit oversize/empty/missing/network/API-rejection error reporting), an inbound `document` handler that saves uploads to `/workspace` (or `tempfile.gettempdir()` outside Docker) with sanitized filenames and a path-aware prompt, a `!sendfile <path>` user command for explicit on-demand sends, and an auto-send hook in `_bg_runner` that mails any file written by the `Write` tool — FIFO-paired with the in-flight `file_path`, skipped on `Error:` / `Denied:` results, and de-duplicated per turn so parallel writes don't double-mail. **Inline-keyboard permission UX**: `ask_input_interactive` now takes `options=[(label, value), ...]`, `ask_permission_interactive` ships `[(✅ Approve, y), (❌ Reject, n), (✅✅ Accept all, a)]`, the bridge renders them as a real `inline_keyboard` with `callback_data="cc:<prompt_id>:<value>"`, and the poll loop's `allowed_updates` is widened to `["message", "callback_query"]` with `_handle_callback_query` routing clicks back through the existing `tg_input_event` mechanism (auth-checked, stale-prompt-id dropped, `answerCallbackQuery` clears the spinner, original message edited to append `✓ Selected: y`). 27 new pytest cases under `tests/test_telegram_bridge.py` — no real network calls. `--accept-all` was a red herring; the bridge simply lacked the upload code path.**
